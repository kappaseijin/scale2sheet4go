...
- **決定済み（2026-08-03）**: 二重取得の論点は**案A で確定**した。
...
- **決定済み（2026-08-03）**: 先方は Issue #9 で**案B（暫定）を採り、案C（恒久）は採らない**。
  したがって先方の公開契約は当面変わらず、**排他なし・JSONL の公開は atomic でない・一括 atomicity なし**が残る。
  当方ユーザーは**案Z**（受け入れたうえで、atomic 公開・完了判定を将来の公開契約として残す）を選択した。
...
| AT-09 | `scale2sheet run --period morning` | 入力フォルダにディレクトリ・当日ファイルなし | 空配列扱い、正常終了（**2026-08-03 の決定により、Slice 2 以降は `failed:input-missing` / 終了コード 1 へ変更。#49**） |
