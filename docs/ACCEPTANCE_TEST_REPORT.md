...
| AC-11 | Slice 3 | 自動 | `npx vitest run test/cli/installation.test.ts test/installation/executor.test.ts` | 200c8e8 | 2026-08-10T21:07:06+09:00 | uninstall output | PENDING | 残置absolute path、runtime artifact、manual rebuild+purge commandは `installation.test.ts` がassert。削除側は `applyOperations` が **到達した operation を `[<status>] <describeOperation>`（status は done / skipped / failed）** として出力し（`src/installation/executor.ts:225-243`）、その**書式**は `test/installation/executor.test.ts:125` が守る（中央 logger を除去する変異で KILLED、2026-08-11 実測）。**ただし uninstall CLI の AC-11 composition（bootout / plist / created-paths / manifest / binary ＋ 残置 ＋ purge が一つの利用者経路に揃うこと）を直接検査する試験が無い**ため PENDING とする（Issue #265） |
