...
## 設計が landing したことと、実装が入ったことは違う（2026-08-11）

**cutover の当日、同じ未実装の値に 2 回言及し、2 回目に条件が落ちて誤りになった。同じ人物である。**

| 回 | agmsg 発言時刻 | 内容 |
| --- | --- | --- |
| 1 | 2026-08-11T15:49:32+09:00 | 「次の実行は evening **21:10 または 23:40**（**#261 の既定値が入っていれば**）」 |
| 2 | 2026-08-11T17:51:03+09:00 | 「**21:10 の run が hang した場合**」（**条件を落として断定した**） |

**1 は条件つきで、正しい。**（`#261` は設計のみ landing、実装は未了）
**2 は条件が落ちている。**

**本番の実測値は `21:00` / `23:30`**（`plutil -p ~/Library/LaunchAgents/….evening.plist`）。

```
**条件つきの推測を 1 度書くと、次に自分でそれを事実として使う**
```

**manager 側にも同じ危険が在ったが、agmsg の履歴で確認できる誤りは無い。**
**2026-08-11T16:09:40+09:00 の #276 依頼は「実装は release train の後」と書いており、
2026-08-11T16:08:29+09:00 の #278 依頼は planner.ts の現行値を検証対象にしている。**
**どちらも未実装を区別している。**

### 本日 landing した設計と、その実装状況

```
#184  readiness gate                 **実装済み**（a586a5c）
#243  requestedCellCount と部分転記   **未実装**
#246  ファイル単位スキップ            **未実装**
#182  partialInput の三値化           **未実装**
#242  通知結果の記録                  **未実装**
#251  launchd の失敗診断              **未実装**
#259  設定可能な schedule             **未実装**
#259  morning の no-data semantics    **未実装**
#280  Sheets の timeout               **未実装**（設計は 915a193 に landing）
```

**9 本のうち実装済みは 1 本だけである**（2026-08-11 の cutover 当日時点）。

**設計書が main に在ることと、その挙動が本番で起きることは、別である。**

### 判別の手順

```
**設計書を読んで挙動を語るときは、実装の有無を先に確かめる**
  git log で該当の実装 commit を探す
  あるいは production の実体を引く（plist / binary / status）
```
