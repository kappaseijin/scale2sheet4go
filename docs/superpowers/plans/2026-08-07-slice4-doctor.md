...
updated: "2026-08-09T17:45:00+09:00"
status: proposed
---
...
> **source との乖離ではない。**
> **決定 2-b により、本 Slice では実装しない。** 別 Issue とする。
...
- [ ] **Step 3: 実装する。**
- [ ] **Step 4: テストが通ることを確認する。**
- [ ] **Step 5: 負のコントロール N-3 から N-7 を実行する。**
...
- [ ] **Step 3: §5 の負のコントロール 7 件をすべて実行し、期待どおり FAIL することを確認する。**
...
| S-5 | §5 の負のコントロール 7 件すべてで、対応する試験が FAIL する | Task 5 Step 3 の出力 |
...
> | 2（配備検査） | **2-b。** Slice 4 に含めない。別 Issue へ |
...
| **2-b（推奨）** | **含めない。別 Issue とする** | Slice 4 の範囲が設計書と一致したままになる。**配備検査は #157 の D3（図と実装の一致）でも埋まらない穴**であり、独立した価値がある |
...
**配備検査（決定 2 で除外）は本 Slice で実装しない。** 別 Issue の起票は manager 判断。
