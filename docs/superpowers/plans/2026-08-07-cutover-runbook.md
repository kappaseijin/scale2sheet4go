...
updated: "2026-08-09T18:25:00+09:00"
status: proposed
basis:
  - Issue #114
  - PR #129
...
## 改訂 2026-08-09 — 前提が 2 つ変わり、判定が 3 つ壊れていた

**初版（2026-08-07）から、cutover の前提が変わった。**
**加えて、初版の判定のうち 3 つは、実測したところ対象を測っていなかった。**

| # | 変更 | 節 |
| --- | --- | --- |
| 1 | **順序が確定した。** 先方の PR #83 → 当方の呼び出し撤去（#160、済）→ 連続 2 日観測 | §0 |
| 2 | **G-2 は排他条件である。** 当方が exporter を呼ぶ限り原理的に満たせない | §0 |
| 3 | **観測日程が確定した**（2026-08-10 / 08-11） | §0 |
| 4 | **§2 の停止判定を差し替えた。** 「ファイルの有無」では本日の失敗モードを捕まえられない | §2 |
| 5 | **§3 の `jq` が誤っていた。実文書に対して常に PASS を返す** | §3 |
| 6 | **§6 の `jq` が誤っていた。`null` を返す** | §6 |
| 7 | 前提条件の値が古かった（96 tests → 253 tests 等）。doctor が**未実装**である旨を明示 | §前提条件 |
...
| 日付 | 位置づけ |
| --- | --- |
| 2026-08-09 夕方 | **前哨戦。gate に数えない** |
| 2026-08-10 | 観測 1 日目 |
| 2026-08-11 | 観測 2 日目 |
| 2026-08-11 夜 | **判定** |

**前哨戦を gate に数えない理由**: #160 の配備が同日中であり、
**当方の呼び出しが止まった状態で丸 1 日が経過していない。**
「当方が書いていない」ことを保証できない日を、帰属の観測に使わない。
...
実行前に以下がすべて満たされていること。**2026-08-09 時点の状況を併記する。**

| 項目 | 確認方法 | 2026-08-09 の状況 |
| --- | --- | --- |
| ユニットテスト全 PASS | `npm test`（**bun が PATH に必要**） | **充足**（253 tests / 25 files） |
| Slice 3 install コマンド | `src/cli/installation.ts` に `.command("install")` | **充足**（`:365` / `uninstall` は `:376`） |
| binary provenance 検査 | **`npm test` に統合済み**（#128） | **充足** |
| **doctor 実装完了** | `doctor` サブコマンドが登録されている | **未充足。Slice 4 は未マージ** |
| shadow 受け入れ試験 PASS | `npm run acceptance:pipeline-shadow` | **隔離実行できない**（Issue #168） |
...
> **doctor は cutover の前提に置かれているが、まだ無い。**
> **前提を満たさないまま cutover を実行しない。**
> doctor を前提から外すなら、**外す判断を記録すること**（黙って進めない）。
...
> なお同 acceptance は現在**隔離実行できない**（#148 の早期 `requireGoogleSheetsConfig` で
> poison 検査の前に失敗する。**Issue #168**）。**いずれにせよ毎日の前提にはできない。**
...
> **注意**: `pipeline` 経路へ移ると、**両 period が health を失った文書で
> 片方の period の alert が配送されない欠陥がある**（Issue #165）。
> **cutover 前に #165 を修正することを推奨する。**
> 修正前に cutover した場合、**「通知されたと記録されているのに通知が来ない」**が起こりうる。
...
- [ ] 前提条件をすべて確認した（**doctor 未実装の扱いを決めた**）
- [ ] step 1：先方の LaunchAgent 登録と PR #83 配備の報告を受けた
- [ ] step 2：**2026-08-10** — P-1（配備物の sha256 が `8c3c181…` と一致）を確認した
- [ ] step 2：**2026-08-10** — O-1（先方の終了コードを独立に読む）と O-2（mtime）を取得した
- [ ] step 2：**2026-08-11** — P-1 を確認した
- [ ] step 2：**2026-08-11** — O-1 と O-2 を取得した
...
**準拠**: Issue #114 / PR #129 / PR #136 / PR #160 / Issue #165
