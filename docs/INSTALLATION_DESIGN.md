...
updated: "2026-08-09T17:10:00+09:00"
---
...
### 実行経路の判定（2026-08-09 追記）

> **出所**: 2026-08-09 ユーザー決定（manager 経由で伝達）。
> **決定そのものは reviewer の一次資料による検証範囲の外にある。**
> 検証してほしいのは、**本節が既存の 14 項目・8 失敗段階と矛盾しないこと**である。

`doctor` は plist の `ProgramArguments` を読み、**どちらの実行経路が登録されているかを判定する。**

| 経路 | plist が起動するもの |
| --- | --- |
| **legacy** | `/bin/bash` + チェックアウト内の `scripts/run-pipeline.sh` |
| **installed** | `<prefix>/bin/scale2sheet pipeline --period <period>` |

**判定結果によって、次の 2 項目の報告を変える。**

| 項目 | legacy 経路 | installed 経路 |
| --- | --- | --- |
| 二つの plist の**固定チェックアウトパスの不在** | **`WARN`。** cutover 前として正常であり、`FAIL` にしない | 設計どおり検査する |
| `pipeline-status.json` の直近開始・完了・結果 | ファイルが無いのは**正常**。「この経路では書かれない」と報告する | ファイルが無いのは**異常**。「未実行」と報告する |

#### なぜ経路で報告を変えるか

**変えないと、`doctor` は cutover の前提として使えない。**

cutover 手順書は `doctor` を cutover の前提条件に置いている。
一方、cutover 前の plist は定義上チェックアウト内のスクリプトを起動する。
**したがって「固定チェックアウトパスの存在」を無条件に `FAIL` とすると、
`doctor` が PASS しないと cutover できず、cutover しないと `doctor` は PASS しない。**

**`pipeline-status.json` についても同じ形の誤診が起きる。**
`run` 経路はこのファイルを書かない。
**「ファイルが無い＝未実行」と一律に報告すると、毎日動いている系を「動いていない」と報告する。**

#### この追記が変えないもの

- **14 の検査項目を増減させない。** 報告の解釈を経路で分けるだけである
- **8 つの失敗段階を増減させない**
- **read-only の制約を変えない。** plist は読むだけであり、書き換えない
