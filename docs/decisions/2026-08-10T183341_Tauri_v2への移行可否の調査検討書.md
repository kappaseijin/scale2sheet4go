...
| 種別 | 対象 | 行数 |
| --- | --- | ---: |
| 実行shell | `scripts/run-binary-source-drift-acceptance.sh`、`scripts/run-installer-acceptance.sh`、`scripts/run-pipeline-shadow-acceptance.sh`、`scripts/run-runtime-safety-acceptance.sh`、`scripts/run-bun-binary-smoke.sh` | 1,063 |
| drift checker | `scripts/check-binary-source-drift.py` | 73 |
| Vitest wrapper | `test/acceptance/pipeline-shadow.test.ts`、`test/acceptance/runtime-safety.test.ts`、`test/acceptance/installer.test.ts`、`test/acceptance/binary-drift.test.ts`、`test/acceptance/bun-binary-smoke.test.ts` | 198 |
| isolated-build meta-test | `test/acceptance/isolated-binary-builds.test.ts` | 22 |
| 合計 | 12 files | 1,356 |

再集計では、上表のpathを明示して`wc -l`へ渡す。

`test/acceptance/`全体や診断用testまで含める集計とは比較しない。
...
CLI cold start上限を決める前に、cache条件を測定protocolとして固定する。

少なくともartifact path、buildから初回実行までの手順、同一artifactの実行回数、再起動の有無を記録する。

異なるcache条件のfirst値を一つの上限判定へ混ぜない。
...
6. coldのcache条件を先に固定し、artifact全体のbytes、cold / warm、build時間、最大RSSを同じMacで3回以上測る。
